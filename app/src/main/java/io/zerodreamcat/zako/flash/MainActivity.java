...
public final class MainActivity extends Activity {
    ...
    private final ServiceConnection connection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder binder) {
            ...
            if (App.rootShell == null || !App.rootShell.isRoot()) {
                console.add("ZakoFlash by SoraNeko.");
    ...
    void Backup() {
        ...
        App.rootShell.newJob().add("mkdir -p /data/media/0/ZakoFlash").exec();
        ...
        binding.install.setOnClickListener(v -> {
            ...
            Shell.Result exitBoot = App.rootShell.newJob().add("cat /data/media/0/ZakoFlash/dd_boot.exit 2>/dev/null").exec();
            Shell.Result exitFull = App.rootShell.newJob().add("cat /data/media/0/ZakoFlash/dd_full.exit 2>/dev/null").exec();
            Shell.Result exitFrp = App.rootShell.newJob().add("cat /data/media/0/ZakoFlash/dd_frp.exit 2>/dev/null").exec();
            ...
            Shell.Result ls = App.rootShell.newJob().add("ls -l /data/media/0/ZakoFlash/boot.img /data/media/0/ZakoFlash/mmcblk0_head_32M.bin /data/media/0/ZakoFlash/frp.img 2>&1").exec();
            console.add("生成的文件:");
        ...
        binding.writeFrp.setOnClickListener(v -> {
            ...
            Shell.Result checkFrp = App.rootShell.newJob().add("ls -l /data/media/0/ZakoFlash/frp.img 2>/dev/null").exec();
            ...
            String cachePath = "/data/media/0/ZakoFlash/frp_mod.img";
            Shell.Result copyResult = App.rootShell.newJob().add("cp /data/media/0/ZakoFlash/frp.img " + cachePath + " 2>&1").exec();
            if (!copyResult.isSuccess()) {
                console.add("错误: 复制 frp.img 到缓存失败");
            ...
            Shell.Result exitWriteFrp = App.rootShell.newJob().add("cat /data/media/0/ZakoFlash/dd_write_frp.exit 2>/dev/null").exec();
